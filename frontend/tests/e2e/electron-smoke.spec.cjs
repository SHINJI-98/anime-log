...
test('desktop app supports tracking, sticky mode and notes without Java or an HTTP backend', async () => {
  ...
  try {
    ...
    await expect(page.getByTestId('follow-anime').first()).toHaveText('已追番')
    await expect(page.getByTestId('follow-anime').first()).toBeEnabled()
    await page.getByTestId('follow-anime').first().click()
    await expect(page.getByTestId('follow-anime').first()).toHaveText('追番')
    await expect(page.getByTestId('discover-view')).toBeVisible()
    await page.getByTestId('watchlist-tab').click()
    await expect(page.getByTestId('watch-record-card')).toHaveCount(0)
    ...
    await expect(page.locator('[data-testid="follow-anime"]').first()).toBeEnabled()
