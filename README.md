...
- 桌面版可将“在看”番剧关联到 Bangumi，按章节排期展示预计已播进度、今日更新和下一集日期。
- 应用运行时每小时同步 Bangumi 排期，并可在北京时间 09:00 后发送可关闭、持久化去重的系统通知。
...
- `GET /api/bangumi/search?keyword=...`
...
yuc.wiki 继续提供季度新番资料；Bangumi 只用于用户确认关联后的放送排期。Bangumi 数据是社区维护的日期级排期，界面均标为“预计”，不代表平台已确认实际播出。日期缺失或无效时不会自行推算集数。
...
追番卡片的「关联 Bangumi」会按标题给出候选，也支持粘贴 `bgm.tv/subject/数字` 链接或条目 ID。关联只影响放送资料，不修改已看集数、状态和笔记。默认仅同步“在看”番剧；应用完全退出后不会在后台运行，下次启动会补同步。系统通知可在桌面设置中全局关闭，也可在单部番剧的关联窗口中关闭。
